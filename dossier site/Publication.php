...
$app->post('/admin/publication', function (Request $request) use ($app) {
    ...
    if (!isset($publicationArray->{'ID'})) {
        ...
        $app['db']->executeUpdate($sqlRequest, array(
            ...
            //rajout éléments demandés par Mr. Salva
            isset($publicationArray->{'doi'}) ? $publicationArray->{'doi'} : null,
            isset($publicationArray->{'url'}) ? $publicationArray->{'url'} : null,
            isset($publicationArray->{'institution'}) ? $publicationArray->{'institution'} : null,
            isset($publicationArray->{'howpublished'}) ? $publicationArray->{'howpublished'} : null,
            isset($publicationArray->{'urldate'}) ? $publicationArray->{'urldate'} : null,
            isset($publicationArray->{'isbn'}) ? $publicationArray->{'isbn'} : null,
            isset($publicationArray->{'chapter'}) ? $publicationArray->{'chapter'} : null,
        ));
...
$app->get('/admin/publications/asc', function (Request $request) use ($app) {
    ...
    if ($encoder->encodePassword('Admin connected') !== $attributeArray->{'a'}) {
        var_dump($encoder->encodePassword('Admin connected'));
        var_dump($attributeArray->{'a'});
        //return new Response('Admin not connected', 403);
    }
...
$app->put('/admin/publications/{id}', function (Request $request, $id) use ($app) {
    ...
    //rajout éléments de publications demandés par Mr. Salva
     if (isset($categorieArray->{'doi'})) {
        $sqlRequest .= ', doi = ?';
        array_push($values, $categorieArray->{'doi'});
    }
     if (isset($categorieArray->{'url'})) {
        $sqlRequest .= ', url = ?';
        array_push($values, $categorieArray->{'url'});
    }
     if (isset($categorieArray->{'institution'})) {
        $sqlRequest .= ', institution = ?';
        array_push($values, $categorieArray->{'institution'});
    }
     if (isset($categorieArray->{'howpublished'})) {
        $sqlRequest .= ', howpublished = ?';
        array_push($values, $categorieArray->{'howpublished'});
    }
     if (isset($categorieArray->{'urldate'})) {
        $sqlRequest .= ', urldate = ?';
        array_push($values, $categorieArray->{'urldate'});
    }
     if (isset($categorieArray->{'isbn'})) {
        $sqlRequest .= ', isbn = ?';
        array_push($values, $categorieArray->{'isbn'});
    }
     if (isset($categorieArray->{'chapter'})) {
        $sqlRequest .= ', chapter = ?';
        array_push($values, $categorieArray->{'chapter'});
    }
    $sqlRequest .= ' WHERE ID = ?';
