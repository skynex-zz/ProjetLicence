...
{    
    ...
    public function modifRubriqueAction() {
        ...
        if($rubriqueToModif == null) {
            return new ViewModel(array('rubNotFound' => 'rubrique not found', 'langue' => $langue));
        }
        //Envoi des variables au layout
        if($listeRubriques != null || $rubriqueToModif != null) {
    ...
    public function deleteRubriqueAction() 
    {
        ...
        try {
            ...
            $msgError = 'errordeleterubrique';
            //$this->redirect()->toRoute('admin', array('action' => 'index', 'langue' => $langue), array('query' => array('errorDltR' => $msgError)));       
        }
        //Envoi des variables au layout
